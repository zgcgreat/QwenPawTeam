/**
 * Console multi-tenant plugin — authentication guard (3-level auth chain).
 *
 * Auth priority:
 ...
 *
 * When MULTI_TENANT_ENABLED is false this component is never rendered
...
export default function MtAuthGuard({ children }: AuthGuardProps) {
    ...
    (async () => {
      try {
        ...
        if (ssoToken) {
          ...
          try {
            const info = await mtAuthApi.initWorkspace(ssoToken);
            if (info && !cancelled) storeVerifiedUserInfo(info);
          } catch {
            // init-workspace failed — still pass through;
            // backend falls back to "default" tenant.
          }
