...
LangString qwenpawStopBackendPrompt ${LANG_JAPANESE} "QwenPaw がまだ実行中で、インストーラーが置き換える必要のあるファイルを使用しています。$\n$\nQwenPaw を終了してから（システムトレイをご確認ください）、「再試行」をクリックして続行してください。「キャンセル」をクリックするとインストールを中止します。"
