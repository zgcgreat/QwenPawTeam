# -*- coding: utf-8 -*-
"""Provider extension: makes ProviderManager tenant-aware for credentials.

Strategy
--------
Instead of replacing ProviderManager, we wrap its key methods so that:

- ``get_provider()`` returns a *copy* of the provider with tenant-specific
  API keys overlaid.
- ``update_provider()`` for builtin providers writes to the tenant's secret
  directory instead of mutating the shared registry.
- ``save_active_model()`` / ``load_active_model()`` use tenant paths.
- All other disk-writing methods (``add_custom_provider``, ``fetch_provider_models``,
  ``add_model_to_provider``, etc.) are overridden to persist to the tenant's
  own ``providers/`` directory under ``{SECRET_DIR}/tenants/{tenant_id}/``.

The shared builtin registry (model lists, base URLs, etc.) remains untouched.
Only *credentials* and *custom providers* are per-tenant.
"""
...
class TenantAwareProviderManager:
    """Wraps the real ProviderManager with tenant credential overlay."""
    ...
    def __getattr__(self, name: str):
        ...
        lifespan init — they are system-level and do not need per-tenant
        behaviour.
        """
    ...
    def _get_tenant_paths(self) -> tuple:
        """Return (root, builtin, custom, active_model) paths for current tenant."""
        try:
            ...
            if ctx and ctx != "default":
                tenant_secret = get_tenant_secret_dir(ctx)
                root = tenant_secret / "providers"
    ...
    def _get_tenant_overrides(self) -> Dict:
        """Load per-tenant provider credential overrides from disk."""
        try:
            from .tenant_context import get_current_tenant_id
            from .auth_extension import get_tenant_secret_dir
            ...
            for subdir in ("builtin", "custom"):
                tenant_subdir = tenant_secret / "providers" / subdir
                if tenant_subdir.is_dir():
                    for p_file in tenant_subdir.glob("*.json"):
                        try:
                            with open(p_file, "r", encoding="utf-8") as f:
                                data = json.load(f)
                            overrides[data.get("id", p_file.stem)] = data
                        except Exception as e:
                            logger.warning("Failed to load tenant override %s: %s", p_file, e)
            return overrides
        except Exception:
            return {}

    def _apply_overlay(self, provider):
        """Return a deep-copied provider with tenant credentials applied.

        The tenant override JSON (written by ``update_provider``) contains
        a full ``model_dump()`` of the provider.  We apply every field
        that the provider model actually declares — this ensures that
        ``base_url``, ``generate_kwargs``, etc. are also per-tenant, not
        just ``api_key``.
        """
        ...
        overrides = self._get_tenant_overrides()
        override = overrides.get(getattr(provider, 'id', ''))
        if override:
            # Apply scalar fields from the tenant override that are valid
    ...
    # ------------------------------------------------------------------
    # Core: tenant-aware _save_provider / load_provider
    # ------------------------------------------------------------------

    def _tenant_save_provider(
        self,
        provider,
        is_builtin: bool = False,
        skip_if_exists: bool = False,
    ):
        """Save a provider config to the tenant's providers directory."""
        root, builtin_path, custom_path, _ = self._get_tenant_paths()
    ...
    def _tenant_load_provider(
        self,
        provider_id: str,
        is_builtin: bool = False,
    ):
        """Load a provider config from the tenant's providers directory."""
        root, builtin_path, custom_path, _ = self._get_tenant_paths()
        provider_dir = builtin_path if is_builtin else custom_path
        ...
        try:
            ...
            logger.warning(
                "Failed to load tenant provider '%s' from %s: %s",
                provider_id,
                provider_path,
                e,
            )
    ...
    # ------------------------------------------------------------------
    # Delegated methods (with overlay or tenant routing)
    # ------------------------------------------------------------------
    ...
    # --- Active model (tenant-aware) ---

    def get_active_model(self):
        # Per-tenant: check tenant-specific active_model.json first
        _, _, _, active_path = self._get_tenant_paths()
        try:
            from .tenant_context import get_current_tenant_id
            ctx = get_current_tenant_id()
            ...
            logger.debug("Failed to load per-tenant active model: %s", exc)
    ...
    async def activate_model(self, provider_id, model_id):
        ...
        # Save to tenant-specific path only — do NOT mutate the global singleton
    ...
    def clear_active_model(self, provider_id: str | None = None) -> bool:
        """Clear the active model, removing from tenant directory."""
    ...
    # --- Provider config update (tenant-aware) ---

    def update_provider(self, provider_id: str, config: Dict) -> bool:
        """Update provider config, persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)

        # For default tenant: delegate to the real manager so that the
        ...
        # For non-default tenants: save to tenant path without touching
        # the shared registry — the overlay in _apply_overlay will pick
        # up the new values from the tenant directory on next read.
        if provider_id in getattr(self._real, 'builtin_providers', {}):
            ...
            self._save_provider_to_tenant(provider_copy, is_builtin=True)
            return True
        ...
        provider_copy.update_config(config)
        self._save_provider_to_tenant(provider_copy, is_builtin=False)
        return True

    def _save_provider_to_tenant(self, provider, is_builtin=False):
        """Save provider to tenant directory (used by update_provider)."""
        _, builtin_path, custom_path, _ = self._get_tenant_paths()
        provider_dir = builtin_path if is_builtin else custom_path
        provider_dir.mkdir(parents=True, exist_ok=True)
    ...
    # --- Custom providers (tenant-aware) ---

    async def add_custom_provider(self, provider_data):
        """Add a custom provider, persisted to the tenant directory."""
        ...
        # Save to tenant directory (not global)
        self._tenant_save_provider(provider, is_builtin=False)
        return await provider.get_info()

    def remove_custom_provider(self, provider_id: str) -> bool:
        """Remove a custom provider from both memory and tenant directory."""
        if provider_id in self._real.custom_providers:
            del self._real.custom_providers[provider_id]
            # Also remove from tenant directory
            _, _, custom_path, _ = self._get_tenant_paths()
    ...
    # --- Model management (tenant-aware) ---

    async def add_model_to_provider(self, provider_id: str, model_info) -> object:
        """Add a model to a provider, persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)
        provider = self.get_provider(provider_id)
        if not provider:
        ...
        self._tenant_save_provider(provider, is_builtin=is_builtin)
        return await provider.get_info()

    async def delete_model_from_provider(self, provider_id: str, model_id: str) -> object:
        """Delete a model from a provider, persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)
        provider = self.get_provider(provider_id)
        if not provider:
            from qwenpaw.providers.errors import ProviderError
    ...
    async def update_model_config(self, provider_id: str, model_id: str, config: Dict) -> object:
        """Update per-model config, persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)
        provider = self.get_provider(provider_id)
        if not provider:
            from qwenpaw.providers.errors import ProviderError
    ...
    async def fetch_provider_models(
        ...
        """Fetch models from a provider, optionally persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)
        provider = self.get_provider(provider_id)
        if not provider:
            return []
    ...
    async def probe_model_multimodal(
        ...
        """Probe a model's multimodal capabilities, persisted to tenant directory."""
        provider_id = self._real._normalize_provider_id(provider_id)
        provider = self.get_provider(provider_id)
        if not provider:
...
def patch_provider_manager(real_manager) -> None:
    ...
    original provider with **no** tenant credential overlay — a security
    leak that lets every tenant see the global API key.
    """
    global _real_provider_manager
    _real_provider_manager = TenantAwareProviderManager(real_manager)
    ...
    # goes through the tenant-aware manager.
    _original_get_active_chat_model = ProviderManager.get_active_chat_model

    @staticmethod
    def _tenant_aware_get_active_chat_model():
        """Tenant-aware replacement for ProviderManager.get_active_chat_model."""
    ...
    ProviderManager.get_active_chat_model = _tenant_aware_get_active_chat_model

    logger.info("[multi-tenant/provider] Wrapped ProviderManager with tenant overlays")


async def wrap_provider_for_tenant(app, provider_manager_instance):
    ...
    Returns
    -------
    The tenant-aware wrapper around the provider manager.
    """
