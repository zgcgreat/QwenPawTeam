# -*- coding: utf-8 -*-
"""Token parser for extracting tenant identity from business-system tokens.
...
``auth_extension`` for the tenant ID helpers.
...
1. **JWT payload** — decode the payload (no signature verification) and
   look for the five-tuple fields or a ``sub`` claim containing the
   composite ID.
2. **Direct composite** — treat the token as a
   ``sysId/branchId/vorgCode/sapId/positionId`` string.
...
# Import tenant helpers lazily to avoid circular imports at module level.
# The actual imports happen inside the functions that need them.


def _get_tenant_fields():
    """Lazy-import TENANT_FIELDS from constants."""
    from .constants import TENANT_FIELDS  # noqa: F811
    return TENANT_FIELDS


def _get_build_tenant_id():
    """Lazy-import build_tenant_id from auth_extension."""
    from .auth_extension import build_tenant_id
    return build_tenant_id


def _get_parse_tenant_id():
    """Lazy-import parse_tenant_id from auth_extension."""
    from .auth_extension import parse_tenant_id
    return parse_tenant_id


class TokenParser(abc.ABC):
    """Abstract base class for token parsing.

    Subclass this to implement custom token-to-tenant resolution logic.
    """

    @abc.abstractmethod
    def parse(self, token: str) -> Optional[dict]:
        """Parse *token* and return tenant identity fields.
        ...
        -------
        dict or None
            A dict with keys ``sysId``, ``branchId``, ``vorgCode``,
            ``sapId``, ``positionId``; or ``None`` if unparseable.
        """
...
class DefaultTokenParser(TokenParser):
    ...
    def parse(self, token: str) -> Optional[dict]:
        ...
        if jwt_result is not None:
            logger.debug("DefaultTokenParser: parsed tenant from JWT: %s", jwt_result)
            return jwt_result

        composite_result = self._try_parse_composite(token)
        if composite_result is not None:
            logger.debug(
                "DefaultTokenParser: parsed tenant from composite: %s",
                composite_result,
            )
    ...
    def _try_parse_jwt(token: str) -> Optional[dict]:
        ...
        TENANT_FIELDS = _get_tenant_fields()
        fields = {}
        ...
        if len(fields) == len(TENANT_FIELDS) and all(fields.values()):
            return fields

        sub = payload.get("sub")
        if sub and isinstance(sub, str):
            try:
                parse_tenant_id = _get_parse_tenant_id()
                return parse_tenant_id(sub.strip())
            except ValueError:
                pass
    ...
    def _try_parse_composite(token: str) -> Optional[dict]:
        """Parse a ``sysId/branchId/vorgCode/sapId/positionId`` string."""
        try:
            parse_tenant_id = _get_parse_tenant_id()
            return parse_tenant_id(token.strip())
...
def parse_token_to_tenant_id(token: str) -> Optional[str]:
    """Parse token and return composite tenant ID string."""
    parser = get_token_parser()
    fields = parser.parse(token)
    if fields is None:
        return None
    build_tenant_id = _get_build_tenant_id()
    return build_tenant_id(**fields)


def parse_token_to_tenant_fields(token: str) -> Optional[dict]:
    """Parse token and return tenant fields dict."""
...
def parse_request_tenant_fields(authorization_header: str) -> Optional[dict]:
    """Canonical entry point: parse tenant identity from Authorization header."""
...
__all__ = [
    ...
    "set_token_parser",
    "reset_token_parser",
    "parse_token_to_tenant_id",
    "parse_token_to_tenant_fields",
    "extract_bearer_token",
    "parse_request_tenant_fields",
]
